...
model.overall_training_data.h_load_ori = model.overall_data.h_load(1:num_training,:);
model.overall_training_data.tau_amb_ori = model.overall_data.tau_amb(1:num_training,:);
model.overall_training_data.radiation_ori = model.overall_data.radiation(1:num_training,:);
model.overall_training_data.tau_in_ori = model.overall_data.tau_in(1:num_training,:);
% verification data acquisition
model.overall_verifying_data.h_load_ori = model.overall_data.h_load(num_training+1:end,:);
model.overall_verifying_data.tau_amb_ori = model.overall_data.tau_amb(num_training+1:end,:);
model.overall_verifying_data.radiation_ori = model.overall_data.radiation(num_training+1:end,:);
model.overall_verifying_data.tau_in_ori = model.overall_data.tau_in(num_training+1:end,:);
...
noise_ratio_training = noise_ratio(1:num_training,:);
noise_ratio_verifying = noise_ratio(num_training+1:end,:);
model.overall_training_data.tau_in  = model.overall_training_data.tau_in_ori .* noise_ratio_training(:,1:num_zone);
model.overall_training_data.h_load = model.overall_training_data.h_load_ori  .* noise_ratio_training(:,end-2);
model.overall_training_data.tau_amb = model.overall_training_data.tau_amb_ori .* noise_ratio_training(:,end-1);
model.overall_training_data.radiation = model.overall_training_data.radiation_ori .* noise_ratio_training(:,end);
model.overall_verifying_data.tau_in = model.overall_verifying_data.tau_in_ori .* noise_ratio_verifying(:,1:num_zone);
model.overall_verifying_data.h_load = model.overall_verifying_data.h_load_ori .* noise_ratio_verifying(:,end-2);
model.overall_verifying_data.tau_amb = model.overall_verifying_data.tau_amb_ori .* noise_ratio_verifying(:,end-1);
model.overall_verifying_data.radiation = model.overall_verifying_data.radiation_ori .* noise_ratio_verifying(:,end);
...
tic;
 ...
 sigma = 1.4826*median(abs(nonzeros(r_0)));
 k=0;
 delta_HME = 1.345;
 r=r_0;
 thre = [];
 while k<1000
     ...
     for t = 1:num_training-model_order
         if abs(r(t,1)/sigma)<=delta_HME
             omega(t,1)= 1;
         else
             omega(t,1) = delta_HME/abs(r(t,1)/sigma);
         end
     end
     omega = omega / (sigma^2);
     BCD(model_order, num_training, num_zone,omega);
     r_new = model.var.residual;
     sigma = 1.4826 * median(abs(nonzeros(r_new)));
...
%      k=k+1;
%  end
model.var.xi = W' * model.var.xi_bar;
% toc;
%% Indicator calculation
% indicators on the training data
% model.training.aggregated_tau_in_ori = model.overall_training_data.tau_in*model.var.xi; % 1080*1
xi_true = load("xi_true.mat");
xi_true = xi_true.xi;
model.training.aggregated_tau_in_ori = model.overall_training_data.tau_in_ori*xi_true;
model.training.aggregated_tau_in_pre = zeros(num_training,1); % 360*1
% model.training.aggregated_tau_in_pre(1:model_order,:)=model.training.aggregated_tau_in_ori(1:model_order,:);
model.training.aggregated_tau_in_pre(1:model_order,:) = model.overall_training_data.tau_in(1:model_order,:)*model.var.xi;
for i =model_order+1 :num_training % 逐点预测
    model.training.alpha_term = zeros(1,1);
    model.training.beta_term =zeros(1,1);
    model.training.gamma_term =zeros(1,1);
    model.training.theta_term = zeros(1,1);
    for j = 2:model_order+1
        model.training.alpha_term =  model.training.alpha_term + model.var.alpha(1,j) * model.training.aggregated_tau_in_pre(i-(j-1),1);
    end
    for j = 1:model_order+1
        model.training.beta_term = model.training.beta_term + model.var.beta(1,j) *model.overall_training_data.h_load(i-(j-1),1);
        model.training.gamma_term = model.training.gamma_term+ model.var.gamma(1,j)*model.overall_training_data.tau_amb(i-(j-1),1);
        model.training.theta_term = model.training.theta_term+ model.var.theta(1,j)*model.overall_training_data.radiation(i-(j-1),1);
    end
    model.training.aggregated_tau_in_pre(i,1)=model.training.alpha_term+model.training.beta_term+model.training.gamma_term+model.training.theta_term+model.var.occ(1+mod(i-model_order-1,48),1);
end
% plot(model.training.aggregated_tau_in_ori);hold on; plot(model.training.aggregated_tau_in_pre);
model.training.error = model.training.aggregated_tau_in_ori-model.training.aggregated_tau_in_pre;
model.training.rmse = sqrt(mean(model.training.error.^2));
model.training.mpae = mean(abs(model.training.error./model.training.aggregated_tau_in_ori))*100;
model.training.r2= 1-sum(model.training.error.^2)/sum((model.training.aggregated_tau_in_ori-mean(model.training.aggregated_tau_in_ori)).^2);

% indicators on the verifying data
% model.verifying.aggregated_tau_in_ori = model.overall_verifying_data.tau_in*model.var.xi; % 360*1
model.verifying.aggregated_tau_in_ori = model.overall_verifying_data.tau_in_ori * xi_true;
...
model.verifying.aggregated_tau_in_pre(1:model_order,:) = model.overall_verifying_data.tau_in(1:model_order,:) * model.var.xi;
for i =model_order+1 :num_verifying % 逐点预测
    ...
    model.verifying.aggregated_tau_in_pre(i,1)=model.verifying.alpha_term+model.verifying.beta_term+model.verifying.gamma_term+model.verifying.theta_term+model.var.occ(1+mod(i+num_training-model_order-1,48),1);
end
...
%% data plot
plot([model.training.aggregated_tau_in_ori;model.verifying.aggregated_tau_in_ori]);hold on;
plot([model.training.aggregated_tau_in_pre;model.verifying.aggregated_tau_in_pre]);
ylim([10,35]);
...
function BCD (model_order, num_training,num_zone,omega)
    ...
    model.var.xi_bar_initial = 1 / num_zone * ones(num_zone, 1);    
    model.var.xi_bar = model.var.xi_bar_initial;    
    %% perform iterations
    ...
    model.var.alpha_term = model.tau_in_times_W_summation(model_order+1:end,:)*model.var.xi_bar;    % alpha term in regression model
    ...
    for i = 2 : model_order + 1    % alpha term calculation
        model.var.alpha_term = model.var.alpha_term - model.var.alpha(1, i) * ...
            model.tau_in_times_W_summation(model_order + 1 - (i - 1) : end - (i - 1), :)*model.var.xi_bar;
    end
    ...
    model.cons = [model.cons, model.var.residual ==  model.var.alpha_term ...
        - model.var.beta_term - model.var.gamma_term - model.var.theta_term - model.var.occ];    % residual calculation. What is more, don't forget the model.var.occ term.
    model.obj_1 = norm(model.var.residual(:).*sqrt(omega), 2)^2 + lambda * model.var.xi_bar'*model.W_times_W_trans_summation*model.var.xi_bar;    % set 2-norm residuals as the objective function
    ...
    model.cons = []; 
    model.cons = [model.cons, model.var.occ(49 : num_training - model_order, 1) == model.var.occ(1 : num_training - model_order - 48, 1)];    % constraints for periodic occupant behaviour
    model.cons = [model.cons, model.ones_times_W_summation'*model.var.xi_bar == 1];    % the sum of aggregated coefficient is 1
    % optimization objective
    model.var.alpha_term = model.tau_in_times_W_summation( model_order + 1 : num_training, :)*model.var.xi_bar;    % the definition of tau_in_hat 
    model.var.beta_term = zeros(num_training - model_order, 1);    % beta term in regression model
    model.var.gamma_term = zeros(num_training - model_order, 1);    % gamma term in regression model
    model.var.theta_term = zeros(num_training - model_order, 1);    % theta term in regressino model
    for i = 2 : model_order + 1   
        model.var.alpha_term = model.var.alpha_term - model.var.alpha(i) * ...
            model.tau_in_times_W_summation( model_order + 1 - (i-1): num_training - (i-1), :)*model.var.xi_bar;
    end
    for i = 1: model_order + 1    % the ith item means the (i-1)th order term in ATDM
        model.var.beta_term = model.var.beta_term + model.var.beta(1, i) * ...    % beta term calculation 
            model.overall_training_data.h_load(model_order + 1 - (i - 1) : end - (i - 1),:);
    ...
    model.settings = sdpsettings('solver', mysolver, 'verbose', 0, 'savesolverinput', 1, 'savesolveroutput',1);
    model.obj_2 = norm(model.var.residual(:).*sqrt(omega), 2)^2 + lambda * model.var.xi_bar'*model.W_times_W_trans_summation*model.var.xi_bar;    % set 2-norm residuals as the objective function
    ...
    model.var.xi_bar = value(model.var.xi_bar);    % recover aggregated coefficient xi. This means the BLA allocates xi_bar to each zone, and each zone recover their own aggregated coefficient xi.

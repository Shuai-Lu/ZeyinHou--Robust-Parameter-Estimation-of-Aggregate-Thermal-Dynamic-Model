...
%  k=0;
%  r=r_0;
% %  delta_HME = 2 * median(abs(nonzeros(r)));
%  delta_HME = 0.3;
%  thre = [];
%  while k<1000
%      omega = ones(num_training-model_order,1);
...
kappa=1.4826;
 ...
 BCD(model_order, num_training, num_zone,omega);
 r_0 = model.var.residual; 
%  sigma = kappa * median(abs(r_0-median(r_0)));
 sigma = kappa*median(abs(nonzeros(r_0)));
 k=0;
 delta_HME = 1.345;
 r=r_0;
 thre = [];
 while k<1000
     ...
     BCD(model_order, num_training, num_zone,omega);
     r_new = model.var.residual;
     sigma = kappa * median(abs(nonzeros(r_new)));
%      sigma = kappa * median(abs(r_new-median(r_new)));
     thre = [thre, norm(r_new-r,2)^2];
     if thre(end) < 10^-6
         break;
     end
